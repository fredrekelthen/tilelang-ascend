...
class BinaryOpSpec:
    def __init__(self, name, tile_op, golden, supported_dtypes,
                 ...
                 kernel_1d=None, kernel_buffload=None, kernel_row_slice=None,
                 mismatch_kernels=None, low_priority_dtypes=None):
        self.name = name
...
class _BinaryOpE2E:
    ...
    def test_tensor_op_scalar(self, scalar_val, dtype, target):
        ...
        run_scalar_op(
            self.op.kernel_scalar, 1024, 1024, 128, 128, scalar,
            dtype, target, golden_fn=self.op.scalar_golden,
        )
...
class _BinaryOpBoundary:
    ...
    def test_buffer_size_unvalidated(self, target):
        ...
        torch.testing.assert_close(
            out.cpu().float(), self.op.golden(a, b[:64]).float(), rtol=1e-2, atol=1e-2
        )
